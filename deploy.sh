...
kubectl apply -f k8s
kubectl set image deployments/server-deployments server=marlovil/multi-server$SHA
kubectl set image deployments/client-deployments client=marlovil/multi-client$SHA
kubectl set image deployments/worker-deployments worker=marlovil/multi-worker$SHA
